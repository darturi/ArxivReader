...
struct PaperListView: View {
            ...
            Group {
                if loading {
                    ...
                    List {
                        ...
                        // Load more button
                        if hasMore && searchText.isEmpty && selectedTagFilter == nil {
                            HStack {
                                Spacer()
                                if loadingMore {
                                    ProgressView()
    ...
    private func loadPapers() async {
        ...
        error = nil
        do {
            papers = try await SupabaseService.shared.getUserPapers(userId: userId, list: listType, limit: PAGE_SIZE, offset: 0)
            hasMore = papers.count >= PAGE_SIZE
        } catch {
    ...
    private func loadMore() async {
        guard let userId = authService.userId, !loadingMore else { return }
        loadingMore = true
        do {
            let morePapers = try await SupabaseService.shared.getUserPapers(userId: userId, list: listType, limit: PAGE_SIZE, offset: papers.count)
            papers.append(contentsOf: morePapers)
            hasMore = morePapers.count >= PAGE_SIZE
        } catch {
